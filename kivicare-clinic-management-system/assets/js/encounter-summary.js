(function () {
    ...
    // Preferencia de fallback por AJAX
    ...
    function parseJSONSafe(resp) {
        // WP a veces responde con "0" o HTML; intenta .json() y si falla, intenta parsear texto
        return resp
            .json()
            .catch(async () => {
    ...
    // ====== inyección del botón (CLONAR el botón existente para conservar estilo) ======
    function injectButtonOnce() {
        const buttons = Array.from(document.querySelectorAll("button,a,[role='button']")).filter(
            (el) => !el.closest(".kc-modal")
        );

        // eliminar posibles duplicados previos (no tocar el nuestro)
        buttons.forEach((el) => {
            const t = (el.textContent || "").trim().toLowerCase();
            const isSummary =
                t === "resumen de atención" ||
                t === "resumen de la atención" ||
                (t.includes("resumen") && (t.includes("atención") || t.includes("atencion")));
            const isOurs = el.matches('[data-kc-summary-btn="1"]');
            if (isSummary && !isOurs) el.remove();
        });

        const refBtn = buttons.find((el) => {
        ...
        clone.className = (clone.className || "") + " js-kc-open-summary";
        if (currentId) clone.setAttribute("data-encounter-id", currentId);

        const hasIco = clone.querySelector("i");
        if (hasIco) {
            const ico = hasIco.cloneNode(true);
            clone.innerHTML = "";
        ...
        refBtn.parentNode.insertBefore(clone, refBtn.nextSibling);
    }

    // ====== abrir modal del resumen ======
    ...
    function openSummary(id) {
        ...
        if (!id) {
            alert("No se pudo detectar el ID del encuentro");
            return;
        }
        ...
        fetchJSON(restUrl, REST.headers("GET"), ajaxUrl2, AJAX.headers())
            .then((json) => {
                ...
                wrap.addEventListener("click", (e) => {
                    if (e.target.classList.contains("kc-modal")) wrap.remove();
                });
                ...
                // imprimir (Carta + 1cm todas las hojas, limpiar estilos invasivos, logo/firma, receta a la izquierda)
                ...
                if (printBtn) printBtn.addEventListener('click', async (ev) => {
                    ...
                    // Fondo seguro (dataURL si se puede)
                    ...
                    // Tomar logo/firma de varios sitios
                    ...
                    // Documento de impresión (NO heredamos CSS del sitio para evitar márgenes/padding extra)
                    w.document.write('<!doctype html><html><head><meta charset="utf-8"><title>Resumen de Atención</title>');
                    w.document.write(`
    <style>
      /* Carta (8.5x11in) + margen uniforme 1cm en TODAS las hojas */
      @page { size: Letter; margin: 1cm; }
      @page { size: 8.5in 11in; margin: 1cm; } /* fallback */
      ...
      * { box-sizing: border-box; }
      ...
      /* Contenedor sin padding para que mande @page */
      .kc-print-root {
      ...
      /* Reset DURO: elimina márgenes/padding/anchos que vengan “de fábrica” en el HTML de la modal */
      .kc-print-root, .kc-print-root * {
      ...
      /* Reconstruimos solo lo necesario */
      .kc-print-root h1 { font-size: 18pt; margin: 0 0 8pt !important; }
      .kc-print-root h2 { font-size: 16pt; margin: 10pt 0 6pt !important; }
      .kc-print-root h3 { font-size: 14pt; margin: 10pt 0 6pt !important; }
      .kc-print-root p, .kc-print-root li, .kc-print-root td, .kc-print-root th { font-size: 12pt; }

      .kc-print-root ul { margin-left: 18pt !important; }
      .kc-print-root li { list-style: disc outside; }

      /* Tablas limpias y receta a la izquierda */
      .kc-print-root table { width: 100% !important; border-collapse: collapse !important; table-layout: auto !important; }
      .kc-print-root thead th { border-bottom: 2px solid rgba(0,0,0,.35) !important; }
      .kc-print-root th, .kc-print-root td {
        padding: 6pt 8pt !important; border-top: 1px solid rgba(0,0,0,.2) !important; vertical-align: top !important; text-align: left !important;
      }
      .kc-print-root .kc-prescription th, .kc-print-root .kc-prescription td { text-align: left !important; }
      ...
      /* Footer firma a 1cm, como en tu PHP */
      .kc-footer {
        position: fixed; bottom: 1cm; left: 0; width: 100%;
        text-align: center; line-height: 1.1; z-index: 2;
      }
      .kc-footer img { max-height: 80px; display: inline-block; padding: 10px; }

      /* Aislar Órdenes clínicas */
      @media print {
        .kc-hard-break { break-before: page; page-break-before: always; }
        .kc-isolated-page { break-after: page; page-break-after: always; }
      }
    </style>
  `);
                    ...
                    // Clonar y LIMPIAR el contenido: quitamos estilos inline y hojas <style>/<link> que trae la modal
                    ...
                    // Marcar la tabla de receta para asegurar alineación izquierda
                    ...
                    // Aislar “Órdenes clínicas” + footer con firma (exacto a tu estructura)
                    ...
                    emailBtn.addEventListener("click", (ev) => {
                        ...
                        postEmail(restEmail, ajaxEmail, to)
                            ...
                            .catch(() => {
                                ...
                                alert(
                                    "No se pudo enviar desde el sistema. Se abrió tu cliente de correo con el contenido."
                                );
                            });
                    });
            })
    ...
    // manejador de nuestro botón clonado
    ...
    // -------- Fallback impresión factura (sin tocar estilos existentes) --------
    document.addEventListener("click", (e) => {
