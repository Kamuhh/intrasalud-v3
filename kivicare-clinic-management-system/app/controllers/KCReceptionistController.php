...
class KCReceptionistController extends KCBase {
	...
	public function save() {
        ...
        // Validar que se haya enviado el id_number
        if (empty($request_data['id_number'])) {
            wp_send_json([
                'status' => false,
                'message' => esc_html__('El campo ID es obligatorio.', 'kc-lang')
            ]);
        }

        // Validar que no exista ya un usuario con ese ID como username
        if (username_exists($request_data['id_number'])) {
            wp_send_json([
                'status' => false,
                'message' => esc_html__('Ya existe un usuario con este ID.', 'kc-lang')
            ]);
        }

        $username = sanitize_user($request_data['id_number']);
		...
		if ( empty($request_data['ID']) ) {
            ...
            update_user_meta($user, 'id_number', $request_data['id_number']);
            ...
            update_user_meta($user_id, 'id_number', $request_data['id_number']);
