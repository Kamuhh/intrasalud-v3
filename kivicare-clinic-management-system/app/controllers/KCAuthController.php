...
class KCAuthController extends KCBase {
	...
	public function patientRegister() {
		...
		try {
			...
			//$username = kcGenerateUsername($parameters['first_name']); <<< Así estaba
            $username = sanitize_user($parameters['cedula']);


			$password = kcGenerateString(12);
//Para prevenir errores, puedes verificar que la cédula no esté registrada
            if (username_exists($parameters['cedula'])) {
    wp_send_json([
        'status'  => false,
        'message' => esc_html__( "Ya existe un usuario con esa cédula.", 'kc-lang' )
    ]);
}
// FIN (solo agregué esta línea y ya)
