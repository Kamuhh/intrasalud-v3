...
class KCPatientEncounter extends KCModel
{
    ...
    public static function encounterPermissionUserWise($encounter_id)
    {
        ...
        switch ($login_user_role) {
            ...
            case $kcbase->getDoctorRole():
                // Permitir a todos los doctores ver cualquier encuentro
                $permission = true;
                break;
    ...
    public static function hasEncounterPermission($encounter_id, $action = 'view') {
    $encounter_detail = (new KCPatientEncounter())->get_by(['id' => (int)$encounter_id],'=',true);
    $kcbase = (new KCBase());
    $login_user_role = $kcbase->getLoginUserRole();
    $current_user_id = get_current_user_id();

    // Por defecto, no hay permiso
    $permission = false;
    ...
    switch ($login_user_role){
        ...
        case $kcbase->getDoctorRole():
            // Solo puede EDITAR o ELIMINAR si es quien creó el encuentro
            if ($action === 'edit' || $action === 'delete') {
                if ((int)$encounter_detail->doctor_id === $current_user_id) {
                    $permission = true;
                }
            }

            // Puede VER cualquier encuentro
            if ($action === 'view') {
                $permission = true;
            }
            break;
        ...
        case $kcbase->getPatientRole():
            if ((int)$encounter_detail->patient_id === $current_user_id && $action === 'view') {
                $permission = true;
            }
            break;
    }
