...
function kcPrescriptionHtml($data, $id, $type = "encounter")
{
    ...
    // PARA CONECTAR LOS CAMPOS PERSONALIZADOS DOCTOR CM y MPPS	
    function obtener_custom_field_doctor($doctor_id, $field_id)
    {
        global $wpdb;
    ...
    // Cargar C.I. del paciente
    if (!empty($data->patient_id)) {
        $data->patient_unique_id = get_user_meta($data->patient_id, 'patient_unique_id', true);
    }
    ...
    if (!empty($data->doctor_id)) {
        // Especialidad
        $data->doctor_specialty = ''; // Por defecto vacío
        if (!empty($data->specialties)) {
            $specs = json_decode($data->specialties);
            if (!empty($specs[0]->label)) {
                $data->doctor_specialty = $specs[0]->label;
            }
        }
    }
    ...
    <div>
        ...
        <div class="container-kivicare" style="background: url('https://intrasalud.com/wp-content/uploads/Fondo-recetario.jpg') no-repeat center center; background-size: cover;">
            <div style="font-family: Arial, sans-serif; width: 100%; margin: 30px;">
                <!-- Logo arriba -->
                <div style="width:100%; margin-bottom:20px;">
                    <img src="<?php echo esc_url($data->clinic_logo); ?>" alt="Logo Clínica"
                ...
                <table style="width:100%; border-collapse:collapse; font-size:19px;">
                    <tr>
                        <td colspan="2" style="padding: 3px 0;">
                            <strong>Sr.(a)</strong> <?php echo esc_html($data->patient_name); ?>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding: 3px 0;">
                            <strong>Edad</strong> <?php echo esc_html($data->patient_age); ?>
                        </td>
                        <td style="padding: 3px 0;">
                            <strong>C.I.</strong> <?php echo esc_html($data->patient_unique_id); ?>
                        </td>
                    </tr>
                    <tr>
                        <td colspan="2" style="padding: 3px 0;">
                            <strong>Domicilio</strong> <?php echo esc_html($data->patient_address); ?>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding: 3px 0;">
                            <strong>Ciudad</strong> <?php echo esc_html($data->patient_city); ?>
                        </td>
                        <td style="padding: 3px 0;">
                            <strong>Fecha</strong> <?php echo esc_html($data->appointment_start_date); ?>
                        </td>
                    </tr>
                </table>
                ...
                <div style="width:100%; border-collapse:collapse; font-size:19px;margin-left: 30px;">
                    <span style="font-size:22px; font-style: italic;"><b>Rp.:</b></span>
                    <br>
                    <div style="width:100%; border-collapse:collapse; font-size:17px;">
                        <?php if (!empty($data->prescription)): ?>
                            <?php foreach ($data->prescription as $pre): ?>
                                <div style="margin-left: 30px; margin-bottom: 18px; line-height: 1.1;">
                                    <b
                                        style="text-transform: uppercase; letter-spacing: 1px;"><?php echo esc_html($pre->name); ?></b><br>
                                    <?php echo esc_html($pre->frequency); ?><br>
                                    <?php echo "por " . esc_html($pre->duration) . " días"; ?><br>
                                    <?php echo esc_html($pre->instruction); ?> <br>
                                </div>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </div>
                ...
                $mpps = obtener_custom_field_doctor($doctor_id, 7); // MPPS
                $cm = obtener_custom_field_doctor($doctor_id, 8); // CM
                $cedula = get_user_meta($doctor_id, 'nickname', true);


                ?>
                <!-- Firma y datos del doctor -->
                <div style="position: fixed; bottom: 30px; left: 0; width: 100%; text-align: center; line-height: 1;">
                    <?php
                    $signature = get_user_meta($data->doctor_id, 'doctor_signature', true);
                    if (!empty($signature)) {
                        ?>
                        <div style="display: inline-block; padding: 10px;">
                            <img src="<?php echo esc_html($signature); ?>" alt="signature" style="max-height: 80px;">
                        </div>
                        <?php
                    }
                    ?>

                    <br><b><?php echo strtoupper(esc_html($data->doctor_name)); ?></b><br>
                    <?php echo esc_html($data->doctor_specialty); ?><br>
                    MPPS: <?php echo esc_html($mpps); ?> &nbsp;-&nbsp; CM: <?php echo esc_html($cm); ?><br>
                    C.I. <?php echo esc_html($cedula); ?><br>
                </div>
            </div>
        </div>
    </div>
            ...
            <table style="width: 100%; border-collapse: collapse;">
                <thead>
                    <tr>
                        ...
                        <th
                            style="border-top: 2px solid rgba(0, 0, 0, 0.1); padding: 12px; text-transform: uppercase; text-align: right;">
                ...
                <table style="width: 100%; border-collapse: collapse;">
                    <thead>
                        <tr>
                            ...
                            <th
                                style="border-top: 2px solid rgba(0, 0, 0, 0.1); padding: 12px; text-transform: uppercase; text-align: center;">
                ...
                <tbody>
                    <?php foreach ($data->prescription as $pre) {
                        ...
                        <tr>
                            ...
                            <td
                                style="border-top: 2px solid rgba(0, 0, 0, 0.1); padding: 12px; white-space: wrap; word-break: break-all;">
...
function kcAppointmentPrintContent($themeColor, $appointmentData, $id, $current_user_role = '')
{
    ...
    ?>
    <hr style="height: 1px; background-color: rgba(0, 0, 0, 0.1);">
    </hr>
